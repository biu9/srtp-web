...
class Captcha(object):
    ...
    def joint_image(self):
        ...
        for char in self._chars:
            self.positions[char._char_text] = {}
            _char = char.char
            # _char.show()
            w, h = _char.size
            ...
            if char.position_x is None:
                x = self.get_actual_x(except_x, w)
                char.set_x(x)
                self.positions[char._char_text]['x'] = x

            if char.position_y is None:
                y = self.get_actual_y(except_y, h)
                char.set_y(y)
                self.positions[char._char_text]['y'] = y
